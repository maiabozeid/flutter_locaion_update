...
class MapController extends BaseController {
  ...
  startMission() async {
    ...
    await startLocationTracking(const Duration(seconds: 2)).then((value) {
  ...
  Future<void> startLocationTracking(Duration updateInterval) async {
    positionStreamSubscription = Geolocator.getPositionStream(
      ...
      final newLocation = checkLocation(LatLng(position.latitude, position.longitude));
      locationBuffer.add(newLocation);
      if (locationBuffer.length > bufferSize) {
        locationBuffer.removeAt(0);
      }

      // Calculate the smoothed location as the average of the locations in the buffer
      smoothedLocation = _calculateSmoothedLocation();

      // Update the marker on the map with the smoothed location
      MarkerId('$smoothedLocation');
    });
  }

  LatLng _calculateSmoothedLocation() {
    double sumLat = 0.0;
    double sumLng = 0.0;

    for (var location in locationBuffer) {
      sumLat += location.latitude;
      sumLng += location.longitude;
    }

    final smoothedLat = sumLat / locationBuffer.length;
    final smoothedLng = sumLng / locationBuffer.length;
